...
fn test_expire_claims_bounded_by_max_iter() {
    ...
    // Third sweep with max_iter=100 should process remaining 50 (capped at 50 limit)
...
fn test_expire_claims_skips_no_expiry() {
    ...
    // One claim should remain (the no-expiry one)
    let claims_vec = claims::get_pending_claims(&env, &user);
    assert_eq!(claims_vec.len(), 1);
...
fn test_expire_claims_mixed_expired_valid() {
    ...
    // Advance time to middle of expiry window
    // Default expiry is 30 days; we advance 20 days
...
fn test_expire_claims_idempotent() {
    ...
    // Third sweep should still find nothing
...
fn test_expire_claims_claimable_amount_updated() {
    ...
    // Verify total claimable amount is 6000
    ...
    // Sweep should remove all and reduce claimable amount
    let pruned = claims::expire_claims_bounded(&env, &user, 50);
    assert_eq!(pruned, 3);
